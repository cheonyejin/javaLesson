...
public class InsertDMLTest {
    ...
    public static void param_data(){
        ...
        String sql = "insert into tbl_custom(custom_id, name, email, age, reg_date)" + "values (?,?,?,?,sysdate)"; //자바로 DB에 데이터 넣기
        ...
        public static void fixedData() {
            ...
            String sql = "insert into tbl_custom(custom_id, name, email, age, reg_date)" + "values('nayeon','박나연','na@gmail.com',26,sysdate)"; //자바로 DB에 데이터 넣기
            try (
                //auto close를 위한try형식 - pstmt.close() 필요하므로 try-with-resources사용
                PreparedStatement pstmt = conn.prepareStatement(sql);
